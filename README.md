...
The Magic Eight Ball API allows users to receive random fortune-telling responses typically associated with the toy.

Learn more about Magic Eight Ball API at [github.com/abunchofapis/magic-eight-ball](https://github.com/abunchofapis/magic-eight-ball).
...
This SDK exposes the API as a small set of **semantic entities** — MagicEightBall — that you
...
// Load a specific magiceightball (returns a MagicEightBall)
...
| --- | --- | --- |
| **MagicEightBall** | The MagicEightBall entity (load). | `/magic/JSON/{question}` |
...
magicEightBall, err := client.MagicEightBall(nil).Load(
    map[string]any{"question": "example_question"}, nil,
)
if err != nil {
    panic(err)
}
