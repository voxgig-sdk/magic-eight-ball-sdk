...
import (
	...
	omni "github.com/voxgig-sdk/magic-eight-ball-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MagicEightBallError); is {
				out := map[string]any{
					"name":    "MagicEightBallError",
...
func omniCtx(arg any, client *sdk.MagicEightBallSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MagicEightBallSDK); is {
			client = live
