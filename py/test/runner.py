# MagicEightBall SDK test runner
...
class MagicEightBallTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MagicEightBallTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MagicEightBallTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MagicEightBallTestRunner.getenv("MAGIC_EIGHT_BALL_TEST_LIVE")
        override = MagicEightBallTestRunner.getenv("MAGIC_EIGHT_BALL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MagicEightBallTestRunner.getenv(key)
        ...
        explain = MagicEightBallTestRunner.getenv("MAGIC_EIGHT_BALL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MAGIC_EIGHT_BALL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MagicEightBallTestRunner._test_control is not None:
            return MagicEightBallTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MagicEightBallTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MagicEightBallTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MagicEightBallTestRunner.load_test_control()
...
def load_env_local():
    MagicEightBallTestRunner.load_env_local()


def env_override(m):
    return MagicEightBallTestRunner.env_override(m)


def entity_data(v):
    return MagicEightBallTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MagicEightBallTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MagicEightBallTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MagicEightBallTestRunner.load_test_control()


def live_client_options():
    return MagicEightBallTestRunner.live_client_options()


def live_delay_ms():
    return MagicEightBallTestRunner.live_delay_ms()
