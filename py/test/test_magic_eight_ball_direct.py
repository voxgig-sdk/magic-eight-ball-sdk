...
class TestMagicEightBallDirect:

    def test_should_direct_load_magic_eight_ball(self):
        setup = _magic_eight_ball_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-magic_eight_ball", "live" if setup["live"] else "unit")
        if _skip:
...
def _magic_eight_ball_direct_setup(mockres):
    ...
    if live:
        ...
        client = MagicEightBallSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
