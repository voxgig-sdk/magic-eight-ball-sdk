# MagicEightBall Lua — Agent Guide

The Lua client for the MagicEightBall API. This directory is **generated** — do not edit it by hand; change the model/template/component in `.sdk/` and regenerate. See the [project guide](../AGENTS.md) for the full workflow and the aontu model language.

> Paths below (`.sdk/…`) are relative to the **project root** — one level up
> from this `lua/` directory.
...
npm run generate     # refreshes this lua/ directory
```

Then build and test the generated output:

```bash
# in this target directory (lua/):
luarocks make
make build
make test
```
