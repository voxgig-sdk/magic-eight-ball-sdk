# MagicEightBall TypeScript SDK



The TypeScript SDK for the MagicEightBall API — a type-safe, entity-oriented client with full async/await support.
...
```ts
try {
  const magiceightball = await client.MagicEightBall().load({ id: 'example_id' })
...
| `tester(testopts?, sdkopts?)` | `MagicEightBallSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `MagicEightBallSDK.test(testopts?, sdkopts?)` | `MagicEightBallSDK` | Create a test-mode client. |
...
| `client` | `client(): MagicEightBallSDK` | Return the parent SDK client. |
...
magic-eight-ball/
├── src/
│   ├── MagicEightBallSDK.ts        # Main SDK class
