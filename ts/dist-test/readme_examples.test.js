...
const __1 = require("..");
const SDK_NAME = 'MagicEightBallSDK';
...
const TEST_SEED = { "entity": { "magic_eight_ball": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+MagicEightBallSDK\b/.test(code) ||
        /\bMagicEightBallSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+MagicEightBallSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/MagicEightBallSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
