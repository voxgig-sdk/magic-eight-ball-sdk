...
// MagicEightBall SDK feature test
//
...
class FtCtrl extends MagicEightBallControl
{
...
class FtClient
{
    ...
    private MagicEightBallUtility $utility;

    public function __construct(MagicEightBallUtility $utility, array $options)
    {
    ...
    public function get_utility(): MagicEightBallUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MagicEightBallContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MagicEightBallContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MagicEightBallContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MagicEightBallConfig::make_feature($name);
    ...
    private static function buildUrl(MagicEightBallSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MagicEightBallError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MagicEightBallResponse($fetched);
        }
    ...
    private function fail(MagicEightBallContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MagicEightBallContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
