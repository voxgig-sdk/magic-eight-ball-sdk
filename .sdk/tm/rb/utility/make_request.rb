...
module MagicEightBallUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MagicEightBallResponse.new({})
    ...
    if fetch_err
      ...
      response = MagicEightBallResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MagicEightBallResponse.new(fetched)
    else
