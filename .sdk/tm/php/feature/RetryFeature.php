...
// MagicEightBall SDK retry feature
...
class MagicEightBallRetryFeature extends MagicEightBallBaseFeature
{
    ...
    public function init(MagicEightBallContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MagicEightBallContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MagicEightBallContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MagicEightBallContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
