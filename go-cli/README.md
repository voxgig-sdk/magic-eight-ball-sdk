# magic-eight-ball-cli

boru-driven command-line client **and** interactive REPL for the MagicEightBall
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/magic-eight-ball-cli)
make build

# 2. See usage (words, entities, env vars)
./magic-eight-ball-cli --help

# 3. Provide credentials once, via the environment
export MAGIC_EIGHT_BALL_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./magic-eight-ball-cli load 1 magic_eight_ball            # {id:1} shorthand
./magic-eight-ball-cli load '{id:1}' magic_eight_ball       # explicit match map

# 5. Override the API base URL for a single call
MAGIC_EIGHT_BALL_BASE=https://api.example.com ./magic-eight-ball-cli load 1 magic_eight_ball

# 6. No arguments -> interactive REPL
./magic-eight-ball-cli
magic-eight-ball> load 1 magic_eight_ball
magic-eight-ball> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/magic-eight-ball-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/magic-eight-ball-cli load 1 magic_eight_ball
   ```
...
```sh
./magic-eight-ball-cli load 1 magic_eight_ball          # scalar shorthand for {id:1}
./magic-eight-ball-cli load '{id:1}' magic_eight_ball     # explicit match map
```
...
./magic-eight-ball-cli load 1 magic_eight_ball
```
...
Run with no arguments to open a REPL (prompt `magic-eight-ball>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `magic_eight_ball`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/magic-eight-ball-cli`. |
...
(`./magic-eight-ball-cli <expr>`) and interactively (the REPL), and expressions compose the
