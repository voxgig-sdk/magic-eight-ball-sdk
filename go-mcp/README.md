# magic-eight-ball-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MagicEightBall SDK as
two agent tools — `magic-eight-ball_list` and `magic-eight-ball_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/magic-eight-ball-mcp)
make build

# 2. Provide credentials via the environment
export MAGIC_EIGHT_BALL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user magic-eight-ball \
  -- /absolute/path/to/magic-eight-ball-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./magic-eight-ball-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// magic-eight-ball_list: first page of records
{ "entity": "magic_eight_ball" }
{ "entity": "magic_eight_ball", "query": { } }

// magic-eight-ball_load: one record by id
{ "entity": "magic_eight_ball", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/magic-eight-ball-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user magic-eight-ball \
     -- "$PWD"/dist/*/magic-eight-ball-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `magic-eight-ball_list` and `magic-eight-ball_load` tools now appear
   in new sessions. Ask the agent to *"list magic_eight_ball using magic-eight-ball"*
   and it calls `magic-eight-ball_list` with `{"entity":"magic_eight_ball"}`.
...
```sh
export MAGIC_EIGHT_BALL_APIKEY=sk_live_xxx            # API key
export MAGIC_EIGHT_BALL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `magic-eight-ball_list` tool
...
### Call the `magic-eight-ball_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `magic-eight-ball_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `magic-eight-ball_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MAGIC_EIGHT_BALL_APIKEY` | API key sent with every request. |
| `MAGIC_EIGHT_BALL_BASE` | Optional override of the API base URL. |
...
```sh
./magic-eight-ball-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"magic-eight-ball_load","arguments":{"entity":"magic_eight_ball","query":{"id":1}}}}'
```
