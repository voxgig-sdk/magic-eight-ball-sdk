# MagicEightBall Ruby SDK



The Ruby SDK for the MagicEightBall API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.MagicEightBall` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = MagicEightBallSDK.test(testopts, sdkopts)
```
...
raise a `MagicEightBallError` (a `StandardError` subclass), so wrap
...
```
rb/
├── MagicEightBall_sdk.rb       -- Main SDK module
...
The main module (`MagicEightBall_sdk`) exports the SDK class
