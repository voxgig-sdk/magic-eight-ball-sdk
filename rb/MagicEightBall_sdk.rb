...
require_relative 'MagicEightBall_types'
...
class MagicEightBallSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MagicEightBallHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MagicEightBallHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MagicEightBallFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MagicEightBallUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MagicEightBallHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MagicEightBallHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MagicEightBallHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue MagicEightBallError => err
    ...
    if fetched.is_a?(Hash)
      status = MagicEightBallHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MagicEightBallError.new(
        "graphql_error", "MagicEightBallSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MagicEightBallSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
